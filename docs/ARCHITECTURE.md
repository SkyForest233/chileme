...
> 表中「屏幕」列写的是实现文件名。**除编辑页外，每个路由都有 Miuix 实现**，运行时按 `LocalThemeStyle` 分流；消耗记录页与归档页已于 2026-09-16 合并为**单文件双主题**（`ConsumptionLogScreen` / `ArchiveScreen`，外壳差异走 `ui/components/app/` 的骨架组件），其余 6 对仍是成对的 `Miuix*Screen.kt`（第三批 #3 逐对合并中，顺序见 `devlog/2026-09-16.md`）。无论一份还是两份，业务数据都必须来自同一份 `*State.kt` 状态容器，禁止在 UI 文件里重写业务计算（`ScreenParityTest` 静态拦截）。
...
- **App 级组件层（2026-09-16，第三批 #3）**：屏幕骨架下沉到 `ui/components/app/`，屏幕本体只保留一份业务结构。`AppChrome.kt` = `AppScaffold` / `AppTopBar` / `AppSnackbarHostState`（MD3 与 Miuix 的 `SnackbarHostState` 是两个不相干的类型，容器对外只暴露 `showUndoSnackbar(): Boolean`，免得两个主题的 `SnackbarResult` 顺着签名漏回屏幕层；`isMiuix` 参与 `remember` key，切主题时换容器并让 `LaunchedEffect` 重启，否则撤销条会弹到已卸载的宿主上）；`AppSurface.kt` = `AppCard` / `AppHintText`；`AppListRow.kt` = `AppListRow`（emoji + 右侧强调文本；不认识领域类型，「可删/不可删」由调用方判断后传 `onDelete` 或 `trailingTag`）与 `AppActionRow`（头像槽位 + 恢复/删除两个图标按钮）；`AppControls.kt` = `AppSearchField`（MD3 `OutlinedTextField` 圆角 50 + 放大镜 / Miuix `InputField` 用 label）与 `AppFilterChip`（两版都用 material3 `FilterChip`，只分流色板与标签字号）；`AppConfirmDialog.kt` = `AppConfirmDialog`（MD3 `AlertDialog` 槽位 / Miuix `WindowDialog` + 等宽两个 `TextButton`）。`AppScaffold` 同时取代 `ManageScreens.kt` / `MiuixManageScreens.kt` 里那两份私有的 `*ManageScaffold`（待管理页那一对合并时删除）。
  - **两条从上游源码核出来的硬约束**：① Miuix 的 `Card` 与 `Surface` **不等价** —— `Card`（v0.9.4-rc01）圆角同为 16dp，但内部还套一层 `Column(Modifier.padding(CardDefaults.InsideMargin))` 且 content 是 `ColumnScope`，所以归档行的外壳在 `AppActionRow` 内部各自分流（MD3 `Surface` / Miuix `Card`），**没有**并进 `AppCard`，否则 Miuix 归档行会少一层内衬。② 弹窗必须放在 `AppScaffold` 的 content lambda **里面**并无条件调用、靠 `show` 控制（Miuix `WindowDialog` 的库约束，见 `MiuixDialog.kt`）；MD3 的 `AlertDialog` 放里放外都是独立窗口、渲染无差别，故统一按 Miuix 的要求落位。
