...
└─ ui/
    ...
    └─ screens/                 # 每屏一文件，自带 Scaffold。两层结构（2026-08-22 B-08 起）：
                                ...
                                # ② 渲染层：ConsumptionLog / Archive / FoodDetail / Home / FoodList / Manage（阈值·分类·位置）
                                #    6 对已于 2026-09-16 合并为**单文件双主题**（外壳差异走 ui/components/app/）；
                                #    其余 2 对仍是 XxxScreen.kt（MD3）+ MiuixXxxScreen.kt（Miuix）成对存在 —— Stats / Settings
...
> 表中「屏幕」列写的是实现文件名。**除编辑页外，每个路由都有 Miuix 实现**，运行时按 `LocalThemeStyle` 分流；消耗记录页、归档页、食品详情页、首页、食品列表页、管理页（阈值/分类/位置）已于 2026-09-16 合并为**单文件双主题**（`ConsumptionLogScreen` / `ArchiveScreen` / `FoodDetailScreen` / `HomeScreen` / `FoodListScreen` / `ManageScreens`，外壳差异走 `ui/components/app/` 的骨架组件；首页与列表页那两份由 `NavChrome.kt` 的 pager 直接调用，其余由 `AppNavGraph.kt` 直接调用 —— **该文件已不含任何 `LocalThemeStyle` 分支**），其余 2 对仍是成对的 `Miuix*Screen.kt`（第三批 #3 逐对合并中，顺序见 `devlog/2026-09-16.md`）。无论一份还是两份，业务数据都必须来自同一份 `*State.kt` 状态容器，禁止在 UI 文件里重写业务计算（`ScreenParityTest` 静态拦截）。
...
- **App 级组件层（2026-09-16，第三批 #3）**：屏幕骨架下沉到 `ui/components/app/`，屏幕本体只保留一份业务结构。`AppChrome.kt` = `AppScaffold` / `AppTopBar` / `AppSnackbarHostState`（MD3 与 Miuix 的 `SnackbarHostState` 是两个不相干的类型，容器对外只暴露 `showUndoSnackbar(): Boolean`，免得两个主题的 `SnackbarResult` 顺着签名漏回屏幕层；`isMiuix` 参与 `remember` key，切主题时换容器并让 `LaunchedEffect` 重启，否则撤销条会弹到已卸载的宿主上）；`AppSurface.kt` = `AppCard` / `AppHintText`；`AppListRow.kt` = `AppListRow`（emoji + 右侧强调文本；不认识领域类型，「可删/不可删」由调用方判断后传 `onDelete` 或 `trailingTag`）与 `AppActionRow`（头像槽位 + 恢复/删除两个图标按钮）；`AppControls.kt` = `AppSearchField`（MD3 `OutlinedTextField` 圆角 50 + 放大镜 / Miuix `InputField` 用 label）与 `AppFilterChip`（两版都用 material3 `FilterChip`，只分流色板与标签字号；`AppChipTone` 决定选中态容器色，**只有 `Primary` 两版都显式指定选中态文字色**，其余吃 material3 默认）；`AppConfirmDialog.kt` = `AppConfirmDialog`（MD3 `AlertDialog` 槽位 / Miuix `WindowDialog` + 等宽两个 `TextButton`）；`AppText.kt` = **语义字号档位表** `AppTextScale`（12 档：Hero/Emphasis/Heading/Body/Meta/Hint/Value/Label/SectionTitle/Action/ItemTitle/Tag → 两主题各自的 TextStyle；`Action` 原名 `Link`，第 5 对发现筛选胶囊文案与首页链接是同一映射后改名）+ `AppText` + `AppEmojiText`（不指定 style，因为两主题「不传 style」时的默认正文样式不同）+ 四个主题色访问器；`AppInfo.kt` = `AppDetailRow` / `AppDivider` / `AppLinearProgress` / `AppHistoryNote`（「归档中找到 N 条」这类带图标的弱化说明，图标 MD3 `History` / Miuix `Recent`）；`AppButtons.kt` = `AppBigButton`（emoji + 文案 + 角标）/ `AppEditButton`；`AppChrome.kt` 另有 `AppMessageScreen`（无顶栏的兜底屏）、`AppEditAction` / `AppDeleteAction` / `AppDestructiveAction`（三者共用私有 `AppBarIconButton`，字形按语义传入）与 `AppSnackbarPlacement`（撤销条落位：`SystemBars` = 二级页，`FloatingNav` = 带悬浮导航栏的 Tab 页抬 84dp）；`AppTopBar` 另有 `onClose` / `selectionMode`（多选态顶栏：私有 `AppBarNavIcon` 里 `onClose` 优先于 `onBack`，两者皆 null 时导航槽不渲染任何东西 —— 与两版原来的空槽逐字等价；`selectionMode` **只让 MD3 换底色** `surfaceContainer`）、`AppSelectAllAction`（字形不对称照抄两版：MD3 恒 `SelectAll` 只换无障碍描述，Miuix 已全选时换 `Close`）与 `AppArchiveAction`；`AppSurface.kt` 另有 `AppPaddedCard`（可传 `onClick` = 可点卡片，`tone: AppCardTone` 决定 MD3 底色档）与 `AppStatCard` + `AppStatTone`（首页三张统计卡，配色按语义在内部取）；`AppListRow.kt` 另有 `AppSectionHeader`（分区标题 + 右侧文字链接）；`AppButtons.kt` 另有 `AppWideButton`（整宽图标 + 文案按钮）；`AppControls.kt` 另有 `AppFilterSectionLabel`（筛选面板分组小标题）与 `AppFilterToggle`（筛选胶囊：计数文案 + 转 180° 的箭头 + 激活态配色，**箭头弹簧两主题各一套**——MD3 `MotionSpring.expand<Float>()` / Miuix 库的 `folmeSpring<Float>(0.95f, 0.2f|0.3f)`，`AnimatedContent` 只写一遍）。`AppScaffold` 已取代 `ManageScreens.kt` / `MiuixManageScreens.kt` 里那两份私有的 `*ManageScaffold`（2026-09-16 第 6 对合并管理页时删除）。列表页那一对另带来 `AppControls.kt` 的 `AppFilterSectionLabel` / `AppFilterToggle`；管理页那一对带来 `AppListRow.kt` 的 `AppCardRow`（前导槽 + 标题 + 可选副标题 + 尾部槽，Miuix 侧圆角 24dp）/ `AppLocationIcon` / `AppEditRowAction` / `AppDeleteRowAction`（可点危险色、禁用最弱色）、`AppControls.kt` 的 `AppStepperPill`（减号两版都用 material 字形）、`AppButtons.kt` 的 `AppAddItemButton`（MD3 胶囊 + 加号 / Miuix 纯文字）、新文件 `AppFormDialog.kt` 的 `AppFormDialog` + `AppFormFieldSpec`（带输入框的弹窗，**两主题的截断时机不同、刻意不桥接**）、`AppText.kt` 的 `appFaintColor()` 与 `AppCard` 的 `miuixCornerRadius`；统计页那一对（2026-09-16 第 7 对）带来 `AppSurface.kt` 的 `AppSection`（节标题 + 卡片外壳：MD3 标题在卡片**内**、Miuix 是卡片**外**的库 `SmallTitle` 且文案更短，所以是 `cardTitle` / `sectionTitle` 两个参数；`titleSpacing` 只在 MD3 侧生效）与 `AppStatsListMetrics` + `appStatsListMetrics()`（两版把列表水平边距放在 `contentPadding` 还是放在 item 上，分工相反，三个值必须一起给）、`AppText.kt` 的 `AppMutedText`（弱化正文；`AppHintText` 从此是它的 `Hint` 档特例）与 `appChartColors()` / `appHighestContainerColor()`（**图表调色板是「屏幕侧取色」唯一被承认的例外**，故 `appPrimaryColor` / `appPrimaryContainerColor` 由 internal 放宽为 public；Miuix 色板没有 `tertiary` / `inversePrimary`，两份 8 色清单照抄不统一），以及 `AppStatCard` 的 `onClick`(可空) / `emojiSize` / `valueSpacing` / `valueScale`（首页与统计页的统计卡同构不同参；MD3 侧 `onClick` 为 null 时退回 `Surface`，保持合并前 `MiniStat` 无水波纹、无按钮语义）。
