...
> 表中「屏幕」列写的是实现文件名。**除编辑页外，每个路由都有 Miuix 实现**，运行时按 `LocalThemeStyle` 分流；消耗记录页、归档页、食品详情页、首页已于 2026-09-16 合并为**单文件双主题**（`ConsumptionLogScreen` / `ArchiveScreen` / `FoodDetailScreen` / `HomeScreen`，外壳差异走 `ui/components/app/` 的骨架组件；首页那份由 `NavChrome.kt` 的 pager 直接调用，不再按主题分流），其余 4 对仍是成对的 `Miuix*Screen.kt`（第三批 #3 逐对合并中，顺序见 `devlog/2026-09-16.md`）。无论一份还是两份，业务数据都必须来自同一份 `*State.kt` 状态容器，禁止在 UI 文件里重写业务计算（`ScreenParityTest` 静态拦截）。
...
- **App 级组件层（2026-09-16，第三批 #3）**：屏幕骨架下沉到 `ui/components/app/`，屏幕本体只保留一份业务结构。`AppChrome.kt` = `AppScaffold` / `AppTopBar` / `AppSnackbarHostState`（MD3 与 Miuix 的 `SnackbarHostState` 是两个不相干的类型，容器对外只暴露 `showUndoSnackbar(): Boolean`，免得两个主题的 `SnackbarResult` 顺着签名漏回屏幕层；`isMiuix` 参与 `remember` key，切主题时换容器并让 `LaunchedEffect` 重启，否则撤销条会弹到已卸载的宿主上）；`AppSurface.kt` = `AppCard` / `AppHintText`；`AppListRow.kt` = `AppListRow`（emoji + 右侧强调文本；不认识领域类型，「可删/不可删」由调用方判断后传 `onDelete` 或 `trailingTag`）与 `AppActionRow`（头像槽位 + 恢复/删除两个图标按钮）；`AppControls.kt` = `AppSearchField`（MD3 `OutlinedTextField` 圆角 50 + 放大镜 / Miuix `InputField` 用 label）与 `AppFilterChip`（两版都用 material3 `FilterChip`，只分流色板与标签字号）；`AppConfirmDialog.kt` = `AppConfirmDialog`（MD3 `AlertDialog` 槽位 / Miuix `WindowDialog` + 等宽两个 `TextButton`）；`AppText.kt` = **语义字号档位表** `AppTextScale`（Hero/Emphasis/Heading/Body/Meta/Hint → 两主题各自的 TextStyle）+ `AppText` + `AppEmojiText`（不指定 style，因为两主题「不传 style」时的默认正文样式不同）+ 四个主题色访问器；`AppInfo.kt` = `AppDetailRow` / `AppDivider` / `AppLinearProgress`；`AppButtons.kt` = `AppBigButton`（emoji + 文案 + 角标）/ `AppEditButton`；`AppChrome.kt` 另有 `AppMessageScreen`（无顶栏的兜底屏）、`AppEditAction` / `AppDeleteAction` / `AppDestructiveAction`（三者共用私有 `AppBarIconButton`，字形按语义传入）与 `AppSnackbarPlacement`（撤销条落位：`SystemBars` = 二级页，`FloatingNav` = 带悬浮导航栏的 Tab 页抬 84dp）；`AppSurface.kt` 另有 `AppPaddedCard`（可传 `onClick` = 可点卡片）与 `AppStatCard` + `AppStatTone`（首页三张统计卡，配色按语义在内部取）；`AppListRow.kt` 另有 `AppSectionHeader`（分区标题 + 右侧文字链接）；`AppButtons.kt` 另有 `AppWideButton`（整宽图标 + 文案按钮）。`AppScaffold` 同时取代 `ManageScreens.kt` / `MiuixManageScreens.kt` 里那两份私有的 `*ManageScaffold`（待管理页那一对合并时删除）。
