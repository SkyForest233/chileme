...
5. **构建验证**：`build_apk()` 必须成功；失败则按第 4 节排错后重建，禁止以失败状态交付
...
- 只用 `androidx.compose.material3`，禁止混入 material2
...
- `rememberNavController()` 只在 MainActivity 声明一次，向下传递回调而非 controller 本身
...
- 构建命令：`build_apk()`（内部执行 `./gradlew assembleDebug`）
