...
| **沙箱被重建** ⇒ 本地 git 停在会话基线 | 沙箱的**文件层**与 **`.git`** 是两个来源：`.git` 可能在本轮被重新 clone（`git reflog` 只剩 `clone → checkout` 两三条）、分支指针退回会话基线，而工作区文件仍是上一轮的最新内容 ⇒ `git status` 会列出几十个"已修改"、`git diff` 显示的是"我的改动 vs 基线"，看着像灾难其实是基线错了。⚠️ 这时**两个本能动作都是自我伤害**：`git checkout -- .` 恢复的是 **index**（不是 HEAD）、`git reset --hard` 用的是那个停在基线的 HEAD ⇒ 工作区被真刷成基线。⇒ 正确顺序：**先 `git log --oneline -1` 与 `git ls-remote origin <本分支>` 对齐**，不一致就先 `git fetch origin <branch>` + `git reset --hard origin/<branch>` 把三处（HEAD/index/工作区）统一到远端，再动手。本仓零丢失的唯一原因：**每笔都推了远端** ⇒ "做完一笔就 push"不是习惯而是恢复手段（09-20 #11 收官后实测踩过，见 devlog §3） |
| 沙箱在**轮次中途**重建（比轮首那次更险） | 09-20 同日第二次踩：脚本因语法错误**没改成任何文件**，但后面
  `git add -A && git commit` 照跑 ⇒ 在"本地 HEAD 已退回基线、工作区却是最新内容"的状态下把整个工作区（66 文件 / +5423 行，
  连两份 `tools-tmp-*.py` 草稿都算上）压成一笔挂在基线上，全靠 `git push` 的 `non-fast-forward` 拦住院线。
  ⇒ 两条硬习惯：**每次 `git commit` 之前**都比对一次 `git log --oneline -1` 是否仍是本轮开头的尖端（只在轮首比对不够）；
  **不要把 `git add -A && git commit` 接在可能失败的脚本后面**当"顺手一批"—— 脚本失败时 `add -A` 吞进去的是整片工作区，
  不是你打算提交的那几份（本仓草稿文件混进历史、以及 §4 上面那条"表格行被粘成一行"没被任何门禁拦住，都是同一个毛病）|
| 对遗留 `*.patch` 用 `git apply` | `git apply` 只在**基线对得上**时安全：对不上时它不报错，只是把别处的内容一起改回去。⇒ 除了 `--check`，应用后立刻比对**改动面是否等于补丁的文件数**（`git status --porcelain | wc -l` vs `grep -c "^diff --git" x.patch`）；补丁一旦进过主干就该删掉，留着=第二份真相（09-20 的 `2026-09-17-ci-actions-upgrade.patch` 就是这么处理的）|
