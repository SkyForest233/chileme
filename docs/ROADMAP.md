...
| 6 | 派生数据下沉 VM + `WhileSubscribed` | ⏸ **搁置**（2026-09-19 用户决定 `skip_10c`）：开工前侦察量出 `Eagerly` 顺手承担的安全职责（"读 `.value` 永远是真数据"）全仓有 **15 处**依赖，其中编辑页那 1 条（`EditFoodScreen.kt` 里 `remember(editId) { viewModel.items.value.find { … } }`）**会丢数据** ⇒ 要做必须先加固、再分三档关预热；**范围此前已缩小**，见下 | #10 节 + 09-19 §11 |
...
- **证据（改造前基线，2026-09-18 逐条实测；4a 之后的现值见本节末「4a 落地结果」）**：
  ...
  1. ~~**4 个「可空 StateFlow + 手工 `consume`」** 承载一次性事件~~（**4a 已消除**）：`_undoRequest`（#4a 前在 `AppViewModel.kt`，该属性现已不存在 ⇒ 不写行号）、
     `_deletedConsumption`、`_restoredArchivedEvent`、`_autoSyncMessage`（#4a 前的行号经 #4 / #10b 两轮早已失效 ⇒ 四个都不再写死地址；
     唯一的 `String?` 型，装的是**成功**提示「已自动同步到坚果云 ☁️」）。每条**只有 1 个订阅方**，
     且 **4/4 都调了 `consumeXxx()`**（当时的 `MainApp.kt` 两处、`ConsumptionLogScreen.kt` 一处、`HomeScreen.kt` 一处；这四个调用点也已随 #4a 一并删除）
  ...
  另：**自动同步失败是刻意静默的**（`AppViewModel.kt` 里 `maybeAutoSync()` 的注释原文「失败静默忽略，下次启动重试；不打扰用户」）——
...
- **证据（2026-09-18 逐条实测）**：
  ...
  - **#5c 收官当天实测（2026-09-18）**：`FoodRepository.kt` 行数 252、类级函数 9 个
    ⚠️ 这里刻意不写「现值」：M1-1（拆凭据 DataStore）与 M1-3（归档溢出计数器）各加了 1 个 key 与 1 条读流，
    行数随之变。**现值一律看 `bash tools/doc-metrics.sh` 的「FoodRepository.kt（#5c 后的核心）」那行**（已改成实测）
...
- **全项验收（2026-09-18 收官逐条对账）**：
  ...
  ③ 🔶 **在 #5 范围内达成**：`data/` 下 15 个文件最大 **293** 行、无一超 400（已加进 doc-metrics 当判据。⚠️ 当时的文件数是 15，#11c 拆出 `NutstoreWebdav.kt` 等之后实测 18 个 ⇒ 这个数不要抄，一律现测）。
...
- **⚠️ 原路线图的另一半已经不成立了**：它写的「屏幕仍在组合期筛选/排序」—— 今日实测**被禁的聚合模式在屏幕文件里是 0 处**
  ...
  `StatsScreen.kt` 里 **1 处**（把状态层算好的 `categoryShare` 转成图表要的 `Float`，属绘制适配）。
...
- **⚠️ 2026-09-19 开工前侦察（#10b 收官后）⇒ 用户决定本项搁置**：改法虽然只有一个词
  ...
  - UI 侧 **1 处会丢数据**：`EditFoodScreen.kt` 里的 `remember(editId) { viewModel.items.value.find { … } }`
    ...
    配合着（`MainActivity.kt` 收集 `ready` 那句、用 `splash.setKeepOnScreenCondition { !contentReady }` 按住启动屏
...
- **⇒ 本项要做的顺序（照此走，别上来就改那个词）**：① **加固提交**（前置）：VM 侧 14 处改 `.first()`
  （`Eagerly` 下与 `.value` 同值 ⇒ 行为等价、且不再依赖订阅状态）+ 编辑页 `remember(editId)` 那处**真行为改动**
...
- **证据（资源化）**：`strings.xml` 条目数 vs 含中文的字符串字面量处数——**两个数都不在此手抄**，
  跑 `bash tools/doc-metrics.sh` 看「strings.xml 条目」与「含中文的字符串字面量」两行（本轮 M1 加了几条 Log 文案，手抄的数当天就烂）
...
- **证据（无障碍）**：`contentDescription = null` **50** 处；`Modifier.semantics { }` 真调用 **1** 处
  （`ui/components/UndoSnackbar.kt` 的 `.semantics {` 那处）。⚠️ 多份历史报告写的「7 处 semantics」是**把 6 行 import 算进去了**
...
- **证据（2026-09-18 逐条实测）**：
  - ✅ **已经有的**：`ui/components/DataCorrupt.kt` 的 `DataCorruptBanner` 在 `HomeScreen.kt` 的 `DataCorruptBanner(` 调用处置顶显示，
    用人话报「库存、归档 数据读取失败」+「这部分数据的写入已暂停，其余数据不受影响（2026-09-15 起按 key 粒度降级）」
    +「原始内容已留档到应用私有目录 corrupt/ 下」，并给两个出路：导入此前的备份恢复 / 「放弃这部分数据」
    （`HomeScreen.kt` 的 `AppConfirmDialog(` 是二次确认弹窗）；`corruptKeyNames()` 把 key 翻成中文名，横幅与弹窗共用；
...
- **已做但推不上去**：13 处 action 升级（`upload-artifact` v4→v7.0.1 / `cache` v4→v6.1.0 /
  ...
  `gitleaks-action` →v3.0.0 等）已改好并**静态核对**过（`git apply --check` + `git diff` 逐行看，**不含 CI 验证**），
  但沙箱的 GitHub App 令牌**缺 `workflows` 权限** ⇒ 存为 `docs/audits/2026-09-17-ci-actions-upgrade.patch` 待用户自己应用。
  ⚠️ **09-20 又试了一次，结论没变**：`git apply --check` 在正确 HEAD 上通过、本地提交做完 ⇒ **push 被 GitHub 服务端拒**
  （`remote rejected … refusing to allow a GitHub App to create or update workflow … without 'workflows' permission`，
  报错原文见 `devlog/2026-09-20.md` §2）⇒ 这份补丁**仍只能由用户本人应用**；"已改好并验证过"那半句从此改为
  「已改好并**静态核对**过」—— 它至今没被任何一次 CI 跑过。
...
- **仍挂着的 4 个小项**（都便宜、都不需要 `workflows` 权限之外的东西，但同样要用户点头）：
  ⚠️ **09-20 用户决定：本项整体先不动**，等真要上架 Google Play 时一并处理（含那份 action 升级补丁的 apply ——
  它同样只能由用户推，沙箱令牌改不了 `.github/`，报错原文见 `devlog/2026-09-20.md` §2）⇒ 下面这张表
  **保持原样作为待办清单，但不要再主动提议做其中任何一项**，除非上架窗口打开。
  | 项 | 今日实测 | 价值 |
  | --- | --- | --- |
...
| **10e** ✅ **已做**（2026-09-19） | `EditFoodScreen.kt` 660 行（**1 个** Composable、**0** 处主题分支）按表单区块抽出 **5** 个同包文件：入口 660 → 317（10e 当天实测；M1-2 之后会变，现值看 doc-metrics），新文件行数同看该行（详见「10e 落地结果」） | 每文件 < 400 行 ✅ 6 个文件全达标；屏幕目录内超 400 的 **2 → 0**（10e 收官那天下一个是 1 —— 只剩「明确不动」的 `StatsScreen.kt` 409；09-19 #11d ① 把 66 行图表件下沉 `ui/components/StatsCharts.kt` ⇒ 409 → 327，**现值以本脚本实测为准**） | 纯结构、无主题分岔 ⇒ 守卫面实测**比规划还小**：`ImeHandlingTest` 清单没动、`ScreenParityTest` 豁免集没动 ⇒ **守卫同批改 0 处、可见性放宽 0 处**；⚠️ 但它**不是纯搬运**（刀口是新发明的、约占产出 15%），靠验证器 V1–V4 + eyeball 兜，抓到 1 个缺形参的真 bug（验证器自己先坏两轮）；✅ **用户真机复测通过**（2026-09-19，编辑页七条）⇒ 本轮收官 |
| — | **按行数明确不动**（09-16 的判定，只覆盖「成对主题实现」那一类）：`AppChrome.kt` / `AppListRow.kt` / `AppControls.kt` / `AppText.kt` / `AppSurface.kt` —— 吸收主题差异正是它们的职责，消灭它等于把差异推回屏幕层。⚠️ **09-19 两次改动本行**：① 行数列**删掉**（#11a 把 `AppText.kt` 的取色 helper 抽走后它当场变短，抄来的数一晚上就烂 ⇒ 现值一律看 `doc-metrics`）；② 原行里的 `StatsScreen.kt` 与 `ExpiryCalendar.kt` **移出「不动」名单** —— 它们被 #11 按**职责边界**接手（11d / 11e），理由不是行数 |
...
- **全项验收**：
  ...
  ⚠️ **10e 已于 2026-09-19 落地**（入口 660 → 317，当天实测）⇒ 那条例外**已消失**，两个已知例外现在只剩 `StatsScreen.kt` 一个。
  现口径 = 只认两块靶子（设置页入口与 `AppViewModel`），并把两个已知例外显式记在这里：
  **`StatsScreen.kt`（当时 409、只超 9 行，当时判「不拆」；⚠️ 09-19 立项 #11 后按**职责边界**改判要拆 = 11d ⇒ 本行保留为当时口径）**、**`EditFoodScreen.kt` 660（10e，未排期；659 是 #10b-4 前，那轮它多了 1 行 import）**
  （⚠️ 这两条是当时的实测，数字留原样：**10e 落地后 `EditFoodScreen.kt` 已是 317 行 ⇒ 例外只剩 `StatsScreen.kt` 409 一个**）；
  主代码超 400 行的**全清单**仍由 doc-metrics 那行给出（10a-2 后是 5 个、**10b-4 后是 4 个**：`EditFoodScreen.kt` 660 / `AppChrome.kt` 479 / `StatsScreen.kt` 409 / `AppListRow.kt` 406 —— 后三个在下方「明确不动」表里；**10e 后是 3 个**，`EditFoodScreen.kt` 已出列），不写死在本文件；
  ...
  ⚠️ **本条的家没了，改挂在这里当待办（2026-09-19）**：10c 已由用户决定**搁置**，而 10b 七轮当时的口径是
  「行为零改动 ⇒ 不占用复测、**与 10c 那轮一起过**」⇒ 「10a-2 的组合边界」与「10b 七轮」这两样都失去了
  原本挂靠的那一轮。10e 的复测**只覆盖编辑页**（`EditFoodScreen` 那 12 个新组件边界，2026-09-19 用户真机
  七条全过），**没有**覆盖设置页与 VM 那两片 ⇒ 待办：下次动到设置页 / VM 相关功能时顺带过一眼，
  或用户主动要求时单独走一轮（两主题）。缓解因素照实记：10b 七轮每轮都有 CI + 四个守卫 + 7 项独立复核，
  10a-2 那项此前已从「必测」降级（实测没有 `remember` 跨边界搬家）。
...
- **本地四项**：`kt-lexcheck` **105** 份 0 问题 · `move-importcheck` 新文件 8/8 缺 0、VM 缺 0（疑似多余 6 全是
  类成员遮蔽的已知假警 —— 补注 2026-09-19 #10b-6：复跑同一条命令实为 **7** 条，那 6 条已随成员搬走**归零**；
  多出的 `ChiliMeApp` 成因不同 —— `--new` 跨 3 个目录时 `same_package_decls` 取的是**目录并集**，
  `MainApp.kt` 所在的 `com/agon/app/` 里正好有 `ChiliMeApp.kt` ⇒ 被误当同包声明；工具只加不删 ⇒ 无害）· `guard-mirror` **0** 问题（18 处跳过）· `doc-metrics` 表格断裂 0 / 写死的数不符 **0** /
...
### 10b-6 落地结果（2026-09-19，提交见台账）

- **搬了什么**：领域 6「设置」**6 个函数 / 6 行**（全是一行体：`setAutoSyncDays` 298 · `setDynamicColor` 300 ·
  `setDarkMode` 302 · `setPalette` 304 · `setThemeStyle` 306 · `setFloatingNav` 308；行号是搬运前的）⇒ 新文件
  `viewmodel/AppViewModelSettings.kt` **90** 行 / **8** 条 import；`AppViewModel.kt` **322 → 304** 行
  （非空 **268 → 256**）、import **46 → 40** 条；`viewmodel/` 7 → **8** 个文件（八个全部 < 400）。
  VM 那 **−18** 行 = 6 行函数 + 6 条 data import + 6 行折叠掉的空行，三类闭合。
- **六轮里最薄的一轮**：6 块都是一行体 ⇒ 能逐字校验的"体"是 **0** 行（声明行走逆变换比对）；这 6 个原本夹在
  `maybeAutoSnapshot` 与「本地快照管理」之间、自己没有段标题 ⇒ **一条标题都不删**（那条快照标题底下仍有
  `_localSnapshots` 等成员、不是孤儿；结果上的孤儿检查照跑，剩余 3 条逐条查过）。
- **四个「零」**：① **零放宽**（连着第五轮）—— 只用 `repo`（10b-1 已 `internal`）与 `viewModelScope`
  （androidx 的扩展属性、非类成员）⇒ 放宽总数仍 **7**；⚠️ 10b-7 必须放宽 6 个 ⇒ 累计 **13**（规划交底的数）；
  ② **零常量随迁**；③ **零字面量** —— 6 块里 `"` 出现 0 次 ⇒ "字面量落在哪个文件"类守卫无从变化
  （已固化成复核断言，比逐个点名强）；④ **零守卫改动** —— `SettingsStateTest` 用 `RecordingActions` 假实现
  （不 import viewmodel 包、不提 `AppViewModel`、不读源码文本），`CorruptGuardTest` 读的是 Food / Backup / Cloud
  三个兄弟文件、5 条断言仿真仍全真，`SnackbarCopyTest` 32 个片段分布一字不变。
- **验收②（不改行为）实测**：`git diff --numstat` = VM **+0 / −18** · 新文件 **+90** · 1 个调用方 **+6**。
  **判据：新增行 = 放宽 + 新 import ⇒ 6 = 0 + 6。** 非逐字的改动只有两类：6 条声明行（补 `internal` 与接收者；
  这 6 个原本都不是 `private` ⇒ 可见性未变）、import 行。**独立复核 7 项全过**（`.tmp-verify8.py`，原文取
  `git show HEAD:` 快照）。
- **同名相撞 6 处、一个不漏**（10b 六轮里第一次全撞）：6 个名字与 `data/FoodSettings.kt` 的
  `internal suspend fun FoodRepository.同名` 一字不差 ⇒ 新文件既声明 `internal fun AppViewModel.同名`、
  又必须 `import com.agon.app.data.同名`（体内是 `repo.同名(`）；VM 侧那 6 条 data import 反过来整词 0 出现 ⇒ 删。
- **一笔旧账结掉，同一个限制换了地方**：10b-5 那轮 VM 侧「疑似多余 **6**」点名的正是这 6 个名字 ⇒ 本轮
  **VM 那栏 6 → 0**（`现有 40 · 应有 40 · 缺 0 · 疑似多余 0`）；⚠️ 判据 7 的限制立刻在**调用方**那栏重现
  （`SettingsState.kt` 自己声明 6 个同名 override ⇒ 那 6 行新 import 被判"多余"，实际必须留）。
- **调用面是六轮里最窄的**：跨包 **1 个文件 / 6 处**（`ui/screens/SettingsState.kt` 的 `ViewModelSettingsActions`
  适配器）⇒ **+6 行 import**，按 ASCII 插在 `saveNutstoreCredentials` 与 `syncDownload` 之间（组里那条 10b
  分组注释照旧被容忍、已有行不重排）；5 个屏幕走 `state.<名>(` ⇒ 不需要 import。兜底：全仓扫到的文件集合 ==
  清单、名字集合逐文件相等、同名 import 冲突预检 **0** 处、可调用引用 **0** 处。
- **本地四项**：`kt-lexcheck` **106** 份 0 问题 · `move-importcheck` 三个文件**缺 0**（新文件 8/8 ·
  VM 40/40 疑似多余 **0** · `SettingsState.kt` 疑似多余 27 条 = 参照物池推不出 Compose 文件既有 import 的老噪声，
  其中 6 条正是那批必须留的新 import）· `guard-mirror` **0** 问题（18 处跳过）· `doc-metrics` 表格断裂 0 /
  写死的数不符 **0** / `stateIn(` 仍 **20** 处（**#10c 的靶子一处没碰**）/ `WhileSubscribed` 仍 **0** 处 /
  `corruptedKeys` 仍 **32** 处。⚠️ 本轮 KDoc 第一版写了带括号的 `stateIn(` ⇒ 实测 20 → **21**、三处断言报不符，
  改措辞后回到 20：**被追踪的是带括号/尖括号的形状**（`Channel` 安全、`Channel<` 不安全）。
- **进度**：领域 **6 / 7**（函数 **43 / 50**）；`viewmodel/` 八个文件全部 < 400
...
- **进度**：领域 **7 / 7** ✅ ⇒ **#10b 收官**；#10 只剩 **10c**（= #6：20 处 `stateIn(` 加 `WhileSubscribed(5_000)`，
  ...
  ⚠️ **本段是 10b-7 收官当时的进度，随后两件事都变了**：10c 已由用户决定**搁置**（见 #6 节）、10e 已**落地**（见下节）⇒ #10 现在只剩「搁置」与「延后」两条。

### 10e 落地结果（2026-09-19，提交见台账）

- **切法**：`EditFoodScreen.kt` 是 **1 个** `@Composable`（100–660，561 行的单个函数体、最深 11 档缩进），
  按表单区块抽出 **5** 个同包文件，入口 **660 → 317**（−343，−52%）：`EditFoodFieldSections.kt` **221**
  （分类 / 数量+单位 / 存放位置 / 生产日期 / 保质期 / 备注六个区块，外加 `unitOptions` 与 `shelfLifePresets`
  两个候选值表）、`EditFoodCoverSection.kt` **150**、`EditFoodThresholdSection.kt` **119**、
  `EditFoodChrome.kt` **106**（底栏保存条 + 日期弹窗）、`EditFoodNameSection.kt` **104**
  ⇒ **6 个文件全 < 400**，本行的验收口径达成。抽出的 12 个组件全部**无状态**（value + onValueChange），
  两段有副作用的逻辑（保存、联想一键回填）**留在入口**当 lambda —— 它们分别要读 12 个表单状态、写 9 个字段。
- **三条硬约束换来了两个「零」**：**可见性放宽 0 处**（两个候选值表跟着唯一使用者搬走、仍 `private`；
  `CAMERA_TEMP_TTL_MS` 的使用者 `launchCamera` 留入口）、**守卫同批改 0 处**（对比 10b：放宽 13、守卫改 3 次）。
  约束是：① 12 个 `rememberSaveable` 一个不搬（位置就是进程死亡恢复的 key，见 09-19 §11 那条冷读路径）；
  ② `Scaffold` 与 `imePadding()` 留入口（`ImeHandlingTest:114` 按完整路径点名、清单里的文件不存在 = 直接红）；
  ③ 新文件名不以 `Screen.kt` / `State.kt` 结尾（`ScreenParityTest` 规则 1 只枚举这两种，本文件在豁免集
  `NoSharedStateScreens` 里；规则 2 的 `uiFiles()` 自动覆盖新文件，四个禁用模式实测 0 命中）。
- **屏幕目录口径**（不含 `*State.kt`）：**16 文件 / 4,528 行 → 21 文件 / 4,885 行**（+357 = KDoc + 签名 +
  每文件自己的 import 块，拆分的固有成本）；目录内超 400 的 **2 → 1**（只剩「明确不动」表里的 `StatsScreen.kt` 409）。
- **搬运的账**（`.tmp-verify10e.py` V1 逐行核）：原文 635 个非空行 = **503 行逐字可寻** + **41 行改写**
  （赋值改成回调 / 调用点 / `return@Button` 改 if-else / 毫秒换算包进 `onConfirmDay(…)`）+ **3 行删除**
  （三条区块分隔注释）+ 88 行 import 块整体重算。入口 import **88 → 50**（删 39 未使用、加 `HistoryEntry`）；
  新文件由 `move-importcheck` 推出 **131** 条、手写补 **3** 条 —— ⚠️ `CategoryDef` ×2 与 `HistoryEntry` ×1
  在旧文件里**从未出现过**（`categories` / `suggestions` 都由 `by` 委托下来、源码不写类型），
  而工具的参照物就是旧文件的 import ⇒ 推不出来，**这是本轮量到的工具真实边界**。
- **⚠️ 判据②（不改行为）在本轮要打折**：10a/10b 搬的是**已存在的独立声明**，能断言「一字未改」；
  这里刀口是新发明的（签名 / 调用点 / 两段 lambda，约占产出 **15%**）⇒ 只能验到「区块体逐字 + 账目对平 +
  调用点与形参双向一一对应（V4）+ 新文件里没有无法解释的名字（V2）」。
  为此写的验证器**自己先坏了两轮**：V2 第一版去字符串 ⇒ 漏掉只出现在字符串模板里的 `categories`
  （真 bug：缺形参、编译必红），改成「剥字面量、留模板」后当场报出来（反过来整段留字符串又会把 `"SNACK"`
  当无法解释的大写名假警）；V4 的深度计数把调用自己的左括号算进去 ⇒ 12 个调用全部误报。
  修完拿那个真 bug 当阳性对照，eyeball 也独立抓到同一个 ⇒ 两条路径交叉证实。
- **行为**：零改动，但**重组边界多了 12 个**（lambda 实参每次重组新建 ⇒ 不指望跳过重组，正确性不受影响）
  ⇒ **已请用户真机过一遍编辑页七条**（2026-09-19 **复测通过** ⇒ 本轮收官）：新增 / 编辑 / 联想一键回填 /
  拍照与相册 / 日期弹窗 / 保存后连续录入 / 进程被杀后恢复到编辑页。
- **进度**：#10 = 10a ✅ + 10b ✅ + **10e ✅**；10c ⏸ 搁置（要先加固，见 #6 节）、10d 延后不排期
  ⇒ 本项只剩「搁置」与「延后」两条，没有未落地的可执行项。

---

---

## #11 按**职责边界**的第二轮拆分（2026-09-19 立项；**刻意不设行数门槛**）

> **立项缘由是用户两次纠偏**：① 我拿 `tools/doc-metrics.sh` 的「主代码 > 400 行」当筛子报了一遍"只剩 3 个文件超标"
> ⇒ 用户否掉：「不要遵循死板的 400 行限制，按照职责边界划分」。② 我第一轮只答了"谁超标"，漏了"谁混着几件事"
> ⇒ 重扫范围改成 **≥150 行的 40 个主代码文件逐个过顶层声明清单**（口径：`find app/src/main -name '*.kt' | xargs wc -l`
> 取 ≥150 的那批，再逐文件列 `fun` / `class` / `// ---- 段标题 ----`）。结论：**行数与边界错置基本无关** ——
> 3 个超 400 的行（`AppChrome` 479 / `StatsScreen` 409 / `AppListRow` 406）里只有 1 个值得按行数动，
> 而真正该动的 6 处里有 4 处**都在 400 行以下**（`AppText.kt` 349 就是其中之一）。

### 判据（本项自己的验收口径）

**一件事一个文件**，且满足三条才算数：
1. 搬走的行与新文件**逐行相等**（#10b 的逆变换验证法：声明行剥掉可见性再比）；
2. **调用点 import 改动数 = 预测值**（同包内移动应为 0；跨包每挪一个公开符号，调用方 +1 条 import）；
3. 附一条**「改前必红」的位置守卫**（断言某类声明只允许出现在指定文件里），不是断言行数、不是断言"有 N 个函数"
   —— 后者会随功能增减腐烂，是 `doc-metrics` 一直在治的那种写死数。

### 执行顺序与每笔的证据（数字为 2026-09-19 实测，现值以 `bash tools/doc-metrics.sh` 为准）

| 笔 | 动谁 | 里面混着几件事 | 拆成 | 为什么先/后 |
|---|---|---|---|---|
...
- **按行数拆 `AppSurface.kt` / `AppControls.kt` / `AppListRow.kt`** —— 09-16 那条「吸收主题分支正是它们的职责」的判定
  只覆盖"**成对主题实现的重复**"，我 09-19 引用时把范围放大了（已在本节改窄）。⚠️ 但**它不是"组件层文件不用管边界"的挡箭牌**：
  11a / 11b 那两处就是边界错置，与主题吸收无关，该动还得动。
- **拆 `ManageScreens.kt`**（3 个管理屏一文件）—— `doc-metrics` 的屏幕数口径 + `ScreenParityTest` 只扫 `ui/screens/` 一层
  + `ImeHandlingTest` 按完整路径点名 ⇒ 拆它要同批改三处判定，换回来的只是目录好看。
- **拆 `NavChrome.kt`**（4 套底栏实现在一个文件）—— 双主题成对，属上面第一条的适用范围。
- **把 `Settings*Dialogs.kt` 拆成"一弹窗一文件"** —— 已按弹窗域分开，再细会产出 8 个百行文件。
- **拆 `MiuixDialogContentTest.kt`**（480 行）—— 6 条判据与自检同文件是刻意的，拆它会伤自检的自洽性。
- **合并 `EmptyState`（`Controls.kt`）与 `AppMessageScreen`** —— 前者是带动画的空态块（emoji + 标题 + 副标题），
  后者是整屏消息 + 一个动作按钮，**不是同一件事**；11b-3 只做搬家。**"空态有两种实现"这条 09-19 只写了"另记待办"却没落地**
  ⇒ 09-20 已登记进 `devlog/INDEX.md` 的待办总览（本仓口径：待办只住 INDEX 一份）；属体验类、需真机。

### 11a – 11f 落地记录（09-19 单日六笔；数字与踩坑细节只写在 `devlog/2026-09-19.md` §15 – §20 与 `devlog/2026-09-20.md` §1，此处不重抄）
...
- **每笔统一做法补两步**（这次真红出来的，别省）：
  ...
  3. **搬完**再跑 `python3 tools/kt-name-audit.py <新文件…>`：上面两步都以「原文件的 import 表」为全集，
     所以查不到**新写出来的类型名**（09-19 #11d② 的 `List<Color>` ⇒ 第六次红，CI 报 `Unresolved reference 'Color'.`）。
     它只看签名的类型位置，主代码全仓实测 0 假警；判据与已知边界写在工具头部，自检 6 个对照。

### 风险与同批要处理的守卫

| 风险 | 说明 | 对策 |
|---|---|---|
| 路径敏感的源码守卫 | `CorruptGuardTest` / `AppViewModelReadyTest` / `SnackbarCopyTest` / `ImeHandlingTest` / `BackupRulesTest` 都是**按文件路径读源码**的 ⇒ 搬走一段代码 = 让某条断言读到空串（假绿）或直接红 | 每笔先跑 `tools/guard-mirror.py`（#5c 起就在干这事），再改守卫 |
...
- **遗留（已登记进 `devlog/INDEX.md` 待办）**：`FoodCard.kt` 的两处进度条仍自己分流一份，与组件层
