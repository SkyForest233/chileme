...
| **11** | **按职责边界的第二轮拆分**（2026-09-19 立项，**不设行数门槛**；≥150 行的 40 个主代码文件逐个过声明清单） | ✅ **六笔全部做完**（09-19 单日：11a–11f，全为行为零改动的搬运；只有 11f 动执行位置 ⇒ **待用户在 App 内过「升级后凭据仍在 + 首屏不闪」两条**）。`FoodRepository` 的 key/flow 归位仍押到 Room 决定 | `35479524217`（11f）三 job 全绿；11e `35478453333` 全绿 |
