...
## #5 Repository 拆分 + `Clock` 注入 + 轻量 DI —— 🔶 5a+5b 已落地（2026-09-18），5c 未做
...
- **证据（2026-09-18 逐条实测）**：
  - `FoodRepository.kt` **965** 行 / **47** 个类级函数（行数是 **#5b 之后的快照**：#5b 给主构造加了时钟参数与说明
    ⇒ 950 → 965；#5c 按领域拆完会大幅变小。另有 1 个局部函数 `FoodRepository.kt:420`；47 与 detekt 09-16
...
- **现值**：`FoodRepository.kt` **965** 行 / **47** 个类级函数（5c 拆分对象未动）；数据层 + VM 函数体硬调
