...
- ~~**备份规则两面**~~ ⇒ **✅ 2026-09-19 M1-1 已修**（不是"等定调"，而是要先拆存储文件）：凭据独立成
  ...
  **库存数据第一次进系统备份**；`covers/` 在云备份排除、换机直传放行（09-16 那条"封面变孤儿"的不一致就此消解）；
  ...
  与 `devlog/2026-09-19.md` §14.1。**新增的待办是复测**：备份/恢复这条通道只能真机验（`adb backup` 已废）。
