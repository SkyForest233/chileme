...
> - **结构性（部分已动）**：~~`Common.kt` 拆分~~ ✅ 09-16 已完成；~~第二批 `MainActivity` 拆分（1,123 行）~~ ✅ 09-16 已完成（→ 6 个文件，见 [09-16 日志](2026-09-16.md) §13）；**只剩第三批 9 项，完整方案见 [09-16 日志](2026-09-16.md)「🧭 拆分路线图」**（含每项的前置、实测证据与验收）；仍未动：错误模型统一（`Channel<UiEvent>` 全项目 0 处）、凭据拆库 + 备份规则调整、App 级组件层与页面去重（8 对渲染层约 7,200 行）、字符串资源化 + 无障碍（`contentDescription = null` 61 处 / `semantics` 7 处）、CI 供应链加固与 `versionCode` 改用仓库内版本文件。
