...
> - **仍待做（中）**：批量「移动存放位置」弹窗的键盘避让**已修但未真机复测**（`MainActivity.kt:650`；手势 / 三键导航各验一次：弹窗上移、确认按钮可点、底栏不抬升）；`photoPath` 绝对路径 + `File.exists()` 组合期调用（**行号已漂移：`FoodAvatar.kt:43`、`EditFoodScreen.kt:284`**）；`nutstorePasswordFlow` 明文常驻内存。
