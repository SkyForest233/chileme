...
- 详情页「吃掉一份」无撤销（`FoodDetailState.kt` 的 `changeQuantity`，详情页调它传 `-1`；列表页的减号反而有 6 秒撤销）
- 归档 `take(200)` 静默丢最老记录（#5c 拆分后两处分别在 `data/FoodArchive.kt` 与 `data/FoodConsumption.kt` 的 `.take(200)`；拆分前都在 `FoodRepository.kt`）→ 需独立计数器 + UI 提示
- 恢复路径（导入 / 快照 / 云端）无集成测试
