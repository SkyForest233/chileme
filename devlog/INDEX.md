...
- 🔶 **#5 Repository 拆分 + `Clock` 注入 + 轻量 DI**（**5a ✅ + 5b ✅ + 5c ✅ 全部落地**，均 2026-09-18；
  只剩「行为不变」的最后一道确认 = **待用户真机复测**）—— `FoodRepository.kt` 开工时是 **965 行 / 47 个类级函数**（历史快照），
