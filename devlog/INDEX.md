...
> - **仍待做（中）**：批量「移动存放位置」弹窗（MD3）缺 `decorFitsSystemWindows = false` → 键盘盖住确认按钮（`MainActivity.kt:649`/`:676`，09-16 新发现，排入第二批独立提交）；`photoPath` 绝对路径 + `File.exists()` 组合期调用（**行号已漂移：`FoodAvatar.kt:43`、`EditFoodScreen.kt:284`**）；`nutstorePasswordFlow` 明文常驻内存。
...
> - **结构性（部分已动）**：~~`Common.kt` 拆分~~ ✅ 09-16 已完成；**第二批 `MainActivity` 拆分（1,115 行）与第三批 9 项的完整方案已写入 [09-16 日志](2026-09-16.md)「🧭 拆分路线图」**（含每步的验收、`private→internal` 清单、会撞红的 3 个源读测试）；仍未动：错误模型统一（`Channel<UiEvent>` 全项目 0 处）、凭据拆库 + 备份规则调整、App 级组件层与页面去重（8 对渲染层约 7,200 行）、字符串资源化 + 无障碍（`contentDescription = null` 61 处 / `semantics` 7 处）、CI 供应链加固与 `versionCode` 改用仓库内版本文件。
